...
#include <Arduino.h>
/* DRIFT local patch (not upstream): Arduino core 3.x no longer leaks the
 * newlib <time.h> types into Arduino.h, so clockid_t/timespec must come from
 * <time.h> for the prototype below (which old cores needed; newlib declares
 * clock_gettime itself). Re-apply on refresh. */
#include <time.h>
...
#elif defined(__has_include) && __has_include(<byteswap.h>)
#include <byteswap.h>
#define cpu_to_be16(x)  (bswap_16(x))
#define cpu_to_be32(x)  (bswap_32(x))
#else
/* DRIFT local patch (not upstream): macOS has no glibc <byteswap.h>; the
 * clang/GCC builtins are the equivalent byte swaps. Re-apply on refresh or
 * drop once upstream gains a portable fallback. */
...
#if defined(__has_include) && __has_include(<byteswap.h>) /* DRIFT local patch */
#include <byteswap.h>
#endif
